...
export function CompanyDetail({
  ...
  return (
    <aside style={{ borderLeft: '1px solid #eee', paddingLeft: 16 }}>
      ...
      <button
        onClick={() => onRun(company.id, company.enrichment_status)}
        disabled={isRunning}
      ...
      {enrichment === undefined && (
        <p style={{ color: '#666' }}>Loading enrichment…</p>
      )}

      {enrichment === null && company.enrichment_status === 'failed' && (
        <p style={{ color: '#c53030' }}>
          Last enrichment attempt failed. No result yet.
        </p>
      )}
      ...
      {enrichment && (
        <div>
          ...
          <Field
            label='Employee size'
            value={enrichment.employee_size_bucket}
          />
          ...
          <Field
            label='Source / model'
            value={`${enrichment.source} / ${enrichment.model}`}
          />
          <Field
            label='Enriched at'
            value={new Date(enrichment.enriched_at).toLocaleString()}
          />
        </div>
      )}
    </aside>
